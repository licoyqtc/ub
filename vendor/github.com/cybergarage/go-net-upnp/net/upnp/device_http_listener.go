...
func responseNotFound(httpRes http.ResponseWriter) error {
	writeStatusCode(httpRes, http.StatusNotFound)
	writeServerHeader(httpRes)
	return nil
}
...
func (self *Device) httpGetRequestReceived(httpReq *http.Request, httpRes http.ResponseWriter) bool {
	...
	fmt.Printf("self desc url :%s path :%s\n",self.DescriptionURL,path)
	// Device Description ?
